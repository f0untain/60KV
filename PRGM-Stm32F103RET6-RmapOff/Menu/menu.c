...
int RythmOff;
int RunTime;
uint16_t NumberOfRampPulse;
float StepRamp;
int8_t Intensity;
uint8_t Count1sStart = 0;
uint16_t CountRunTime = 0;
float CountRamp = 0;
...
const char MT_FIRST_VARIABLE[]	  = " 1.Intensity";
const char MT_SECOND_VARIABLE[]	  = " 2.Freq(HZ)";
const char MT_THIRD_VARIABLE[]    = " 3.Ton(mS)";
const char MT_FIFTH_VARIABLE[]	  = " 4.Rythm on";
const char MT_SIXTH_VARIABLE[]	  = " 5.Rythm off";
const char MT_SEVENTH_VARIABLE[]  = " 6.Trun(S)";
const char MT_EIGHTH_VARIABLE[]   = " 7.Contact Us";
/*const char MT_FORTH_VARIABLE[]    = " 4.Ramp(S)";
const char MT_FIFTH_VARIABLE[]	  = " 5.Rythm on";
const char MT_SIXTH_VARIABLE[]	  = " 6.Rythm off";
const char MT_SEVENTH_VARIABLE[]  = " 7.Trun(S)";
const char MT_EIGHTH_VARIABLE[]   = " 8.Contact Us";*/
...
//	VARIABLE_VALUE						STRING_NUMBER				DOWN_LIMIT					  UP_LIMIT
	...
	{.pText = {NON, LOW, MED, HIGH},	.strNum = VOL_LEVEL_LOW,    .downLimit = VOL_LEVEL_LOW,   .upLimit = VOL_LEVEL_HIGH},	/* Voltage Value*/
	{.ival = 8,						    .strNum = 10,			    .downLimit = 5,	              .upLimit = 20},		        /* Frq*/
	{.fval = 2.1f,						.strNum = 10,				.downLimit = 0.1f,			  .upLimit = 4.9f},			    /* time*/
//	{.ival = 5,				            .strNum = 10,               .downLimit = 1,	              .upLimit = 10},		        /* Ramp*/
	{.ival = 10,						.strNum = 20,				.downLimit = 1,	 			  .upLimit = 20},			    /* Rythm on*/
	{.ival = 10,			  	        .strNum = 20,               .downLimit = 1,	              .upLimit = 20},	     	    /* Rythm off*/
	{.ival = 10,			  	        .strNum = 20,               .downLimit = 1,	              .upLimit = 60},	     	    /* Run time*/

};
	
STATE_NEXTVARIABLE StateNextVariable[17] = {
	...
	/*	{ VA_SEVENTH_VARIABLE, ROTARY_CW_DIR,   VA_EIGHTH_VARIABLE,  LCD_LINE4 },
		{ VA_SEVENTH_VARIABLE, ROTARY_CCW_DIR,  VA_SIXTH_VARIABLE,   LCD_LINE3 },

		{ VA_EIGHTH_VARIABLE,  KEY_BACK,        VA_FIRST_VARIABLE,   LCD_LINE4 },
		{ VA_EIGHTH_VARIABLE,  ROTARY_CCW_DIR,  VA_SEVENTH_VARIABLE, LCD_LINE3 },*/

		{ 0, 0, 0, 0 }
};

STATE_VARIABLE StateVariable[10] = {
	//	VARIABLE      VARIABLE TEXT		    ADJUST_STATE     P_VALUE_STR		Element TYPE
	{ VA_VOLTAGE, MT_FIRST_VARIABLE, ST_STR_CHANGE, &values[1], ET_FSTR },
	{ VA_FREQUENCY, MT_SECOND_VARIABLE, ST_INT_ADJUST, &values[2], ET_INT },
	{ VA_TIME, MT_THIRD_VARIABLE, ST_FLT_ADJUST, &values[3], ET_FLT },
	{ VA_RYTHM_ON, MT_FIFTH_VARIABLE, ST_INT_ADJUST, &values[4], ET_INT },
	{ VA_RYTHM_OFF, MT_SIXTH_VARIABLE, ST_INT_ADJUST, &values[5], ET_INT },
	{ VA_RUN, MT_SEVENTH_VARIABLE, ST_INT_ADJUST, &values[6], ET_INT },
	{ CONTACT_US, MT_EIGHTH_VARIABLE, ST_CONTACT_US, &values[7], ET_NULL },
	/*	{ VA_RAMP,        MT_FORTH_VARIABLE,    ST_INT_ADJUST,   &values[4],   ET_INT         },
		{ VA_RYTHM_ON,    MT_FIFTH_VARIABLE,    ST_INT_ADJUST,   &values[5],   ET_INT         },
		{ VA_RYTHM_OFF,   MT_SIXTH_VARIABLE,    ST_INT_ADJUST,   &values[6],   ET_INT         },
		{ VA_RUN,         MT_SEVENTH_VARIABLE,  ST_INT_ADJUST,   &values[7],   ET_INT         },
		{ CONTACT_US,     MT_EIGHTH_VARIABLE,   ST_CONTACT_US,   &values[8],   ET_NULL        },*/
...
void DisplayRunMenu(void)
{
	...
	if (flagRefreshValue500ms == true)
	{
		...
		for (uint8_t n = 0; n < 4; n++)
		{
			...
			case ET_FLT:	
				{
					int value = (int)(pValues[n]->fval * 10); // Convert to integer with two decimal precision
					sprintf(lcdBuffer, "%d.%01d", value / 10, value % 10);
	...
	if (refreshLcd == true)
	{
		...
		HAL_GPIO_WritePin(GPIOC, Lcd4_Pin | Lcd5_Pin | Lcd6_Pin | Lcd7_Pin, GPIO_PIN_RESET);
		HAL_GPIO_WritePin(GPIOA, LcdEn_Pin, GPIO_PIN_RESET);
		refreshLcd = false;
		//LcdEnLow;
	}
}
...
#define FIRST_LINE_VAR		currentVariable-cursorLine
	...
	else if (input == KEY_START)
	{
		...
		if (enterFunction == 1)  // If it is the first time the button is pushed and the variables of frequency and time-on(duty cycle) are set
		{
			switch (Intensity)
			{
			case(VOL_LEVEL_LOW): RealyLow(1); Led2(1); break;
			case(VOL_LEVEL_MED): RealyMed(1); Led3(1); break;
			case(VOL_LEVEL_HIGH): RealyHigh(1); Led4(1); break;
			}
			flagCapCh = true;
...
/*		else if (variable == VA_RAMP )
		{
			Ramp = (pValues[cursorLine]->ival * 1000) ;
		}*/
		else if (variable == VA_RYTHM_ON /*&& pValues[cursorLine]->ival==3*/)
		{
...
#define DECIMAL_PART	1
	...
	uint8_t value = (uint8_t)(pValues[cursorLine]->fval * 10); // Convert to integer with two decimal precision
	sprintf(lcdBufferIA, "%d.%01d", value / 10, value % 10);
	LcdPuts(14, cursorLine, lcdBufferIA);
...
void InitValueReadFromEeprom(void)
{
	...
	Intensity = values[1].strNum;
	Frequency = values[2].ival;
	TimeOn = values[3].fval;
	RythmOn = values[4].ival;
	RythmOff = values[5].ival;
	RunTime = values[6].ival;
